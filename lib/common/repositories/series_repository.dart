...
class SeriesRepository extends CoreRepository {
  String _screenUri = '';
  ...
  Future<CoreItemListModel> fetchPage(
    ...
    try {
      ...
      if (response.statusCode == HttpStatus.ok) {
        SeriesDataWrapper dataWrapper = modelSerializers.deserializeWith(
            SeriesDataWrapper.serializer, json.decode(response.body));

        var total = dataWrapper.data.total;

        var coreItemModels = dataWrapper.data.results
            .map(
              (Series coreItemModel) => CardModel(
                    id: coreItemModel.id,
